...
use tuai_common::events::ProcessInfo;
use tuai_common::{ProcessEvent, ProcessEventType};
...
impl super::ProcessMonitorBackend for EbpfProcessMonitor {
    fn start(&mut self) -> tuai_common::PlatformResult<()> {
        self.start().map_err(|e| tuai_common::PlatformError::Internal(e.to_string()))
    }

    fn stop(&mut self) -> tuai_common::PlatformResult<()> {
        self.stop();
        Ok(())
    }
    ...
    fn snapshot(&self) -> tuai_common::PlatformResult<Vec<tuai_common::ProcessInfo>> {
        Ok(self.snapshot())
    }
}
